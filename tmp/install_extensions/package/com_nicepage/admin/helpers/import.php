...
class Nicepage_Data_Loader
{
    ...
    /**
     * @var int Default products list category id
     */
    private $_productsCategoryId = 0;
    ...
    private function _loadProducts() {
        ...
        $categoryList = CategoryMapper::find(array('title' => 'Site Products'));
        ...
        if ($this->_replace || count($categoryList) == 0) {
            $data = array(
                'category_name' => 'Site Products',
                'published' => '1',
                'category_parent_id' => 0,
                'virtuemart_category_id' => '',
                'virtuemart_vendor_id' => 1,
            );
            $this->_productsCategoryId = CategoryMapper::create($data);
        } else {
            $this->_productsCategoryId = $categoryList[0]->virtuemart_category_id;
        }
        ...
        foreach ($this->_data['Products'] as &$productData) {
            ...
            $product = array(
                ...
                'published' => '1',
                'slug' => '',
                'categories' => array($this->_productsCategoryId),
    ...
    private function _loadParameters()
    {
        ...
        if (isset($parameters['productsJson'])) {
            $productsJson = json_decode($this->_processingContent($parameters['productsJson'], 'editor'), true);
            $newProducts = $productsJson['products'];
            if ($this->_replace || !isset($config['productsJson'])) {
                $new['productsJson'] = $newProducts;
            } else {
                $currentProducts = isset($config['productsJson']) ? $config['productsJson'] : array();
                ...
                $new['productsJson'] = $currentProducts;
            }
        }
    ...
    private function _fixProductIds($content) {
        ...
        $maxId = 0;
        foreach ($config['productsJson'] as $product) {
            $id = (int) $product->id;
            if ($id > $maxId) {
                $maxId = $id;
            }
