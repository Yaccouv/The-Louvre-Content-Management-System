...
EOF;
    ...
    public function setSectionsHtml()
    {
        ...
        if ($this->_productName) {
            $products = isset($this->_config['productsJson']) ?
                $this->_config['productsJson'] : array();
            if ($this->_productName == 'product-list') {
                $options['siteProducts'] = $products;
                $productFound = count($products) > 0 ? true : false;
            } else {
                foreach ($products as $product) {
                    if ($this->_productName === ('product-' . $product->id)) {
                        $productFound = $product;
                        break;
                    }
                }
                $options['siteProduct'] = $productFound;
            }
            if (!$productFound) {
                $this->_sectionsHtml = '';
                return;
