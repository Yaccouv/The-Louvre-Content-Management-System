...
if (!defined('_DESIGNER_FUNCTIONS')) {
    ...
    function checkAndGetLanguage()
    {
        ...
        $defaultThemeLang = 'en';
        $themeLangs = json_decode('[{"id":"1","name":"en","title":"English"}]', true);
...
function block_sidebar_custom_products_1($caption, $content) {
    ...
    function getCategory($template, $category, $isVm, $isVertical) {
        if ($isVm) {
            $href = JURI::root(true) . '/index.php?option=com_virtuemart&view=category&virtuemart_category_id=' . $category->virtuemart_category_id;
            $title = $category->category_name;
        } else {
            $href = JRoute::_(ContentHelperRoute::getCategoryRoute($category->id));
    ...
    function getChildCategories($template, $children, $isVm, $isVertical) {
        ...
        foreach ($children as $child) {
            $t = $template;
            if ($isVm) {
                $t = str_replace('[[href]]', JURI::root(true) . '/index.php?option=com_virtuemart&view=category&virtuemart_category_id=' . $child->virtuemart_category_id, $t);
                $t = str_replace('[[title]]', $child->category_name, $t);
            } else {
                $t = str_replace('[[href]]', JRoute::_(ContentHelperRoute::getCategoryRoute($child->id)), $t);
